...
public class LocalDatabaseService
{
    ...
    public async Task<List<WorkoutExerciseDisplay>> GetWorkoutExercisesAsync(Guid workoutLocalId)
    {
        ...
        var q =
            ...
            join e in db.Exercises on we.ExerciseLocalId equals e.LocalId
            where we.WorkoutLocalId == workoutLocalId && !e.IsDeleted
            orderby e.Name
            select new WorkoutExerciseDisplay(e.Name, we.Repetitions, we.WeightKg);
    ...
    public async Task<List<WorkoutExerciseManageRow>> GetWorkoutExerciseManageRowsAsync(Guid workoutLocalId)
    {
        ...
        var map = inWorkout.ToDictionary(x => x.ExerciseLocalId, x => x);

        var result = new List<WorkoutExerciseManageRow>(allExercises.Count);

        foreach (var ex in allExercises)
        {
            if (map.TryGetValue(ex.LocalId, out var link))
            {
                result.Add(new WorkoutExerciseManageRow(
                    ex.LocalId,
                    ex.Name,
                    IsInWorkout: true,
        ...
        List<(Guid ExerciseLocalId, bool IsInWorkout, int Repetitions, double WeightKg)> data)
    {
        ...
        var existingMap = existing.ToDictionary(x => x.ExerciseLocalId, x => x);

        foreach (var row in data)
        {
            if (!row.IsInWorkout)
            {
                if (existingMap.TryGetValue(row.ExerciseLocalId, out var toRemove))
                    db.WorkoutExercises.Remove(toRemove);

                continue;
            }

            if (existingMap.TryGetValue(row.ExerciseLocalId, out var link))
            {
                link.Repetitions = Math.Max(0, row.Repetitions);
                link.WeightKg = Math.Max(0, row.WeightKg);
            }
            else
            {
                db.WorkoutExercises.Add(new LocalWorkoutExercise
                {
                    ...
                    WeightKg = Math.Max(0, row.WeightKg)
                });
            }
        }
